...
@pytest.fixture
def user_create():
    return UserCreate(username="testuser", password="@secret123", password_confirm="@secret123", email="testuser@example.com")
...
def test_get_user_found(db, user):
    with patch("src.backend.services.user_service.UserCRUD.get_by_id", return_value=user):
...
def test_get_user_not_found(db):
    with patch("src.backend.services.user_service.UserCRUD.get_by_id", return_value=None):
        service = UserService(db)
        result = service.get_user(999)
        assert not result.success
        assert isinstance(result.value, AppException.UserNotFound)

def test_create_user_success(db, user_create, user):
    with patch("src.backend.services.user_service.UserCRUD.get_by_username", return_value=None), \
...
def test_create_user_duplicate(db, user_create):
    with patch("src.backend.services.user_service.UserCRUD.get_by_username", return_value=True):
        service = UserService(db)
        result = service.create_user(user_create)
        assert not result.success
        assert isinstance(result.value, AppException.UsernameAlreadyExists)
